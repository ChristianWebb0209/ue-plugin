...
[System context](docs/architecture-maps/system-context.svg)
...
[Plugin containers](docs/architecture-maps/plugin-containers.svg)
...
[Context components](docs/architecture-maps/context-components.svg)
...
[Harness components](docs/architecture-maps/harness-components.svg)
...
[Tooling components](docs/architecture-maps/tooling-components.svg)
...
[Tool surface graph](docs/architecture-maps/tool-surface-graph.svg)
...
[Tool surface sequence](docs/architecture-maps/tool-surface-sequence.svg)
...
[Retrieval components](docs/architecture-maps/retrieval-components.svg)
...
[Memory components](docs/architecture-maps/memory-components.svg)
...
[UI components](docs/architecture-maps/ui-components.svg)
...
[Request lifecycle](docs/architecture-maps/request-lifecycle.svg)
...
[Vector db end to end](docs/architecture-maps/vector-db-end-to-end.svg)
...
[Vector db index build](docs/architecture-maps/vector-db-index-build.svg)
...
[Vector context unified](docs/architecture-maps/vector-context-unified.svg)
...
[Vector db query sequence](docs/architecture-maps/vector-db-query-sequence.svg)
...
- **Tool reliability vs flexibility:** a broad tool catalog increases capability, but requires strict dispatch gating and clearer failure surfaces to keep behavior predictable.
- **Context quality vs token budget:** richer context helps outcomes, but budget pressure forces ranking, packing, and fallback strategies.
- **Latency vs responsiveness:** streaming, tool calls, and editor updates must avoid blocking UX; hot paths need profiling and staged work.
- **Feature velocity vs maintainability:** fast iteration can leave deprecated paths; periodic cleanup and consolidation are required to keep the codebase healthy.

## Current focus

- Improve intermittent tool-flow failures (especially complex Blueprint paths).
- Reduce editor freezing and improve perceived responsiveness.
- Tighten chat/transcript rendering behavior and simplify fragile animations.
- Finish partially stubbed workflows with clear support boundaries.

Known gaps and backlog are tracked in repo issues and docs.
