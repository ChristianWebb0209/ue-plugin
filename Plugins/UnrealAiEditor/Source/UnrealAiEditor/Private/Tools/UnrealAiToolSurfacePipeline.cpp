...

{
	...
	Index.RebuildForEnabledTools(*Catalog, Request.Mode, Caps, PackOptions, ToolFilter);
...

	{
		...
		UnrealAiBlueprintBuilderToolSurface::MergeBlueprintCoreToolsAfterGuardrails(
	...
	int32 ExpandedCount = UnrealAiRuntimeDefaults::ToolExpandedCount;

	int32 Budget = UnrealAiRuntimeDefaults::ToolSurfaceBudgetChars;

	int32 ParamsExcerptMax = 900;

	if (Request.bBlueprintBuilderTurn)

	{

		const int32 BpBudget = BlueprintBuilderAppendixBudgetChars > 0

			? BlueprintBuilderAppendixBudgetChars

			: UnrealAiRuntimeDefaults::BlueprintBuilderToolSurfaceBudgetChars;

		UnrealAiBlueprintBuilderToolSurface::GetVerboseAppendixSettings(
