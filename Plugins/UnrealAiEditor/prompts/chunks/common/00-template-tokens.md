...
| `{{SPECIALIST_DELEGATION_BRIEF}}` | harness (`LastProductSpecialistDelegationBrief` → `FUnrealAiPromptAssembleParams::SpecialistDelegationBrief`) | Verbatim orchestrator handoff body for the active **product specialist** sub-turn; injected via **`chunks/specialists/00-delegation-brief-token.md`**. Empty → placeholder sentence in `ApplyTemplateTokens`. Cleared when the specialist returns or on Blueprint/Environment builder handoff. |
