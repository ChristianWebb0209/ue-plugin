...
**Layout + merge (append new logic to existing events like Tick instead of duplicating):** in-process **`FUnrealBlueprintGraphFormatService`** (`Plugins/UnrealAiEditor/.../Private/BlueprintFormat/`) drives post-**`blueprint_graph_patch`** **`auto_layout`**, **`layout_scope`** (`patched_nodes` \| **`patched_nodes_and_downstream_exec`** \| **`full_graph`**), optional **`layout_anchor`** (`neighbor` \| **`below_existing`**), and standalone **`blueprint_format_graph`**. The layered DAG path in **`BlueprintGraphLayeredDagLayout`** is Sugiyama-style (crossing reduction), not per-node placement during each op.
...
| Field | Value |
|-------|--------|
| **summary** | Structured **`ops[]`** on a **`/Game`** script graph: **`create_node`**, **`create_comment`** (`member_node_refs` + reflow), **`connect`**, **`break_link`**, **`splice_on_link`** (insert on one exec edge), **`set_pin_default`**, **`add_variable`**, **`remove_node`**, **`move_node`**. Optional **`auto_layout`** (default **true**), **`layout_scope`** **`patched_nodes`** (default) \| **`patched_nodes_and_downstream_exec`** (exec-downstream BFS union, capped) \| **`full_graph`**, and **`layout_anchor`** **`neighbor`** (default) \| **`below_existing`** (ignored when scope is **`full_graph`**). Resolver validates scope/anchor; arg repair aliases e.g. **`downstream`** → **`patched_nodes_and_downstream_exec`**, **`below`** → **`below_existing`**. Formatter options follow Editor Preferences → Unreal AI Editor → Blueprint Formatting. **`compile`** default true. |
...
| **returns** | Success: `ok`, `applied[]`, `blueprint_status`, `compiled`, **`auto_layout`**, **`layout_scope`**, **`layout_anchor`**, **`layout_applied`**, **`layout_local_skipped`**, **`layout_hints_ignored`**, **`layout_downstream_extra_nodes`**, **`layout_downstream_truncated`**, optional **`layout_warnings[]`**, **`layout_nodes_positioned`** / **`layout_nodes_moved`** / other **`layout_*`** metrics, **`formatter_available`**. Failure: `status` **`patch_errors`**, `errors[]`, **`error_codes[]`**, **`applied_partial`** always **`[]`** (transaction cancelled), **`note`**, **`suggested_correct_call`**. Invalid **`layout_scope`** / **`layout_anchor`** → resolver **`validation_failed`** or dispatch error before apply. |
